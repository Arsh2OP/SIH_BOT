...
def text_to_audio_filename(response_text):
    # Check if the response text exists in the mapping
    if response_text in response_audio_mapping:
        return response_audio_mapping[response_text]
    return None

def play_audio(response_text):
    audio_file = text_to_audio_filename(response_text)
    if audio_file is not None:
        response_file = os.path.join(RESPONSES_DIRECTORY, audio_file)
        if os.path.exists(response_file):
            ...
            print(f"Response file '{audio_file}' not found.")
    else:
